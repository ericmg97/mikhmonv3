...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',"); {:local date [ /system clock get date ];:local year [ :pick $date 7 11 ];:local month [ :pick $date 0 3 ];:local comment [ /ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :local ucode [:pic $comment 0 2]; :if ($ucode = "vc" or $ucode = "up" or $comment = "") do={ /sys sch add name="$user" disable=no interval="' . $validity . '"; :delay 2s; :local exp [ /sys sch get [ /sys sch find where name="$user" ] next-run]; :local getxp [len $exp]; :if ($getxp = 15) do={ :local d [:pic $exp 0 6]; :local t [:pic $exp 7 16]; :local s ("/"); :local exp ("$d$s$year $t"); /ip hotspot user set comment=$exp [find where name="$user"];}; :if ($getxp = 8) do={ :local gmt (-([/system clock get gmt-offset ] - (2147483647* 2 )-2)/3600); :local inter "' . $validity . ' $gmt h"; /sys sch set $user interval=$inter; :delay 1s; :local dexp [ /sys sch get [ /sys sch find where name="$user" ] next-run]; :local dgetxp [len $dexp]; :if ($dgetxp = 8) do={ :local gmt (-([/system clock get gmt-offset ] - (2147483647* 2 )-2)/3600); :local hour [pick [ /system clock get time] 0 2];:local nhour (hour + gmt);:if ($nhour >= 24) do={ :local dexp [ /sys sch get [ /sys sch find where name="UTC_Fix" ] next-run]; :local getxp [len $dexp];:if ($dgetxp = 15) do={:local d [:pic $dexp 0 6];:local s ("/"); /ip hotspot user set comment="$d$s$year $exp" [find where name="$user"];};};:if ($nhour < 24) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"];};}; :if ($getxp > 15) do={ /ip hotspot user set comment=$exp [find where name="$user"];}; /sys sch remove [find where name="$user"]';
